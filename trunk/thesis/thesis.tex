...
This thesis is a continuation of my bachelor thesis~\cite{Kucbel2011thesis} called \textit{Generování textu z MathML}. The bachelor thesis introduces a program for generating plain text from MathML as described in the paragraph above, however it has serious limitations when it comes to scalability, supports only a subset of the Presentation MathML elements and a limited amount of mathematical operations. The program uses XSL Transformations\footnote{Extensible Stylesheet Language Transformations \url{http://www.w3.org/TR/xslt}} to convert MathML markup into plain text. In this thesis we will try to improve the computation speed, provide better scalability, support a greater amount of mathematical operations and encompass most of the Presentation MathML as well as the Content MathML elements.
...
Example of conversion result can be found in Appendix~\ref{appendix:conversionexamples}.
...
We will compare these implementations by their performance 
